...
import Model from '../stockin/Model';
...
function Form1(props) {
    ...
    const onSubmit = (data) => {
        if (items.length > 0) {
            ...
            axios.post(apidata.api + 'inventory/remove',
                { data: payload },
                { headers: { "Authorization": `Bearer ${token}` } })
                .then(data => {
                    console.log(data)
                    NotificationManager.success(t('inventory_removed'), '');
                    clearForm();
                })
                .catch(error => {
        ...
        <Box direction='row'>
            ...
            <Paper elevation={5} >
                <form onSubmit={handleSubmit(onSubmit)}>
                    <Stack>
                        <TitleTypograpy sx={{ paddingLeft: 2, paddingRight: 2, marginTop: 1, marginBottom: 0, fontSize: 18, variant: 'bold' }}>{t('pinkbug_stock_stockout_remove')}</TitleTypograpy>
