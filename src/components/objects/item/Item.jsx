...
export const Item = () => {
  ...
  const fetchRecords = () => {
    setLoading(true);
    axios.get(apidata.api + 'item', { params: { page: page, size: size, searchkey: searchkey }, headers: { "Authorization": `Bearer ${token}` } })
      .then(response => {
  ...
  return (
    ...
    <Container component="main" style={{ background: '#efefef', minHeight: '100vh' }} >
      ...
      <h4 sx={{ color: '#0d47a1' }}>{t('pinkbug_items')}</h4>
      <Grid container>
        <Grid item sm={8}>
          <Paper elevation={5} style={{ minHeight: 50, paddingTop: '0px' }}>
            <SearchForm allpayment='all_items' />
          </Paper>
          <Paper elevation={5} style={{ minHeight: 600, marginTop: 20, paddingTop: '0px', paddingBottom: '10px', background: '#fafafa' }}>
            <Grid container>
              <Grid item sm={12} sx={{ marginBottom: 1 }}>
                <Model edit={false} refresh={refresh} title='Item' />
              </Grid>
              <Grid item sm={12}>
                <Box>
