...
import { Typography, Table, TableHead, TableBody, TableRow, TableCell, Pagination, Divider, Container, Grid, Paper, Button } from "@mui/material";
...
import RecievdList from './RecievdList';
...
export const StockIn = () => {
  ...
  const fetchRecords = () => {
    setLoading(true);
    axios.get(apidata.api + 'inventory/orderin', { params: { page: page, size: size, searchkey: searchkey }, headers: { "Authorization": `Bearer ${token}` } })
      .then(response => {
  ...
  return (
    ...
    <Container component="main" style={{ background: '#efefef', minHeight: '100vh' }} >
          ...
          <Grid container>
            <Grid item sm={5}>
              <Button style={{ marginLeft: 10, marginBottom: 0, marginTop: 10 }} variant="outlined" color="primary" onClick={() => changeContent(2)}>
                <Add style={{ marginRight: 10 }} />{t('received')}
              </Button>
      ...
      </Grid>
      {
        list && <Grid container>
          <Grid item sm={12}>
            <Paper elevation={5} style={{ minHeight: 50, padding: '2px' }}>
              <TitleTypograpy sx={{ margin: 2, marginBottom: 0, fontSize: 18, variant: 'bold' }}>{t('pinkbug_stock_stockin_list')}</TitleTypograpy>
