...
function Form(props) {
    ...
    const onSubmit = (data) => {
        if (!edit) {
            axios.post(apidata.api + 'vendor',
                { data: data },
                { headers: { "Authorization": `Bearer ${token}` } })
                .then(data => {
                    NotificationManager.success(t('member_created'), '');
                    props.close();
                })
                .catch(error => {
                    NotificationManager.error(t('member_not_created') + error.code, '');
                })
        } else {
            axios.put(apidata.api + 'vendor',
                { data: data, id: props.item.id },
                { headers: { "Authorization": `Bearer ${token}` } })
                .then(data => {
                    NotificationManager.success(t('vendor_created'), '');
                    props.close();
                })
                .catch(error => {
                    NotificationManager.error(t('vendor_not_created') + error.code, '');
                })
        }
    }
    ...
    return (
        <form id="childform" onSubmit={handleSubmit(onSubmit)}>
            ...
            <Box direction='row'>
                <Stack>
                    <TitleTypograpy sx={{ marginTop: 2, marginBottom: 2 }}>{t('add_vendor')}</TitleTypograpy>
