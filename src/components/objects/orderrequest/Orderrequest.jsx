...
import { TableBody, TableHead, Table, TableRow, TableCell, Typography, Pagination, Divider, Box, Container, Grid, Paper, List, ListItemAvatar, ListItem, Avatar, ListItemText, Button } from "@mui/material";
...
export const Orderrequest = () => {
  ...
  const fetchRecords = () => {
    setLoading(true);
    axios.get(apidata.api + 'order', { params: { page: page, size: size, searchkey: searchkey }, headers: { "Authorization": `Bearer ${token}` } })
      .then(response => {
        ...
        list && <Grid container>
          <Grid item sm={12}>
            <Paper elevation={5} style={{ minHeight: 50, paddingTop: '10px', paddingRight:10,paddingLeft:10 }}>
              <TitleTypograpy sx={{ marginTop: 1, marginBottom: 0, fontSize: 18, variant: 'bold' }}>{t('pinkbug_sales_request_list')}</TitleTypograpy>
              <Divider sx={{ marginTop: 1, marginBottom: 3, background: blue[900] }} />

              <SearchForm allpayment='all' />
            </Paper>
            <Paper elevation={12} style={{ minHeight: 600, paddingTop: 0, paddingBottom: '10px', background: '#fafafa' }}>
              <Grid container>
                      ...
                      {records.length > 0 && records.map(x => {
                        return <>
                          <TableRow
                            ...
                            <TableCell>{x.nsCustomerId}</TableCell>
                            <TableCell>{format(new Date(x.order_date), 'MM/dd/yyyy')}</TableCell>
                            <TableCell>{format(new Date(x.order_expected_date), 'MM/dd/yyyy')}</TableCell>
                            <TableCell>{format(new Date(x.completed_date), 'MM/dd/yyyy')}</TableCell>
                            <TableCell align='right'>{x.payment_mode}</TableCell>
                            <TableCell align='right'>{x.gross_amount}</TableCell>
                            <TableCell align='right'>{x.discount_amount}</TableCell>
                            <TableCell align='right'>{x.net_amount}</TableCell>
                            <TableCell align='right'><ModelItems order={x.id}/></TableCell>
                            <TableCell align='right'><ModelItems order={x.id}/></TableCell>
                          </TableRow>
                        </>
                      })}
